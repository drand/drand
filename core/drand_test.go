...
func TestDrandDKGFreshWithReader(t *testing.T) {
	n := 5
	nbRound := 2
	period := 1000 * time.Millisecond
...
func TestDrandDKGFreshWithSameReader(t *testing.T) {
	n := 5
	nbRound := 2
	period := 1000 * time.Millisecond

	file, err := os.Create("veryrandom.txt")
	if err != nil {
		panic(err)
	}
	_, err = file.WriteString("Hey, good morning, Monstropolis. It's now five after the hour of 6:00 A.M. in the big monster city.")
	if err != nil {
		panic(err)
	}
	defer file.Close()
	defer os.Remove("veryrandom.txt")

	reader := "veryrandom.txt"
	userOnly := true
	entropyReader := entropy.NewEntropyReader(reader, userOnly)
