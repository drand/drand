...
func mockResolver(t *testing.T, txtRecords map[string][]string) *madns.Resolver {
...
func TestResolveDNS(t *testing.T) {
	...
	ais, err := resolveAddresses(context.Background(), addrs, mockResolver(t, txtRecords))
	if err != nil {
		t.Fatal(err)
	}
	if len(ais) != 2 {
...
func TestResolveDNSNoAddrs(t *testing.T) {
	...
	_, err := resolveAddresses(context.Background(), addrs, mockResolver(t, txtRecords))
	if err == nil {
