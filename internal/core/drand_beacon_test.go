...
func TestMigrateMissingDKGDatabase(t *testing.T) {
	...
	node := ts.nodes[0]
	beaconDKGStoreDir := path.Join(node.daemon.opts.configFolder, common.MultiBeaconFolder, beaconName)
	...
	node.daemon.dkg = dkg.NewDKGProcess(
		dkgStore,
		node.daemon,
		node.daemon.completedDKGs,
		node.daemon.privGateway.DKGClient,
		node.daemon.privGateway.ProtocolClient,
		dkg.Config{},
		node.daemon.log,
	)
	require.NoError(t, err)
