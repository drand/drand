...
func runSBMigration(baseFolder string) error {
	...
	if isGroupFound || isKeyFound || isDBFound {
		...
		if fs.CreateSecureFolder(path.Join(baseFolder, common.MultiBeaconFolder, common.DefaultBeaconID, key.FolderName)) == "" {
	...
	if isKeyFound {
		oldPath := path.Join(baseFolder, key.FolderName)
		newPath := path.Join(baseFolder, common.MultiBeaconFolder, common.DefaultBeaconID, key.FolderName)
